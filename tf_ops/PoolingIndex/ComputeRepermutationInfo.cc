...
REGISTER_OP("ComputeRepermutationInfo")
        ...
        .Output("reper_p2o_idxs1: int32")              // [pn1]
        ...
        .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
            ...
            c->set_output(1,cens_shape);
            c->set_output(2,o2p_idxs2_shape);
            c->set_output(3,o2p_idxs2_shape);
            c->set_output(4,cens_shape);
            return Status::OK();
        });
...
//        std::cout<<"ComputePermutationInfoWithClassGPUOp begin\n";
        ...
        OP_REQUIRES_OK(context,context->allocate_output(1,TensorShape(dims1),&reper_p2o_idxs1));
        OP_REQUIRES_OK(context,context->allocate_output(2,TensorShape(dims2),&reper_lens));
        OP_REQUIRES_OK(context,context->allocate_output(3,TensorShape(dims2),&reper_begs));
        OP_REQUIRES_OK(context,context->allocate_output(4,TensorShape(dims1),&reper_cens));

        auto reper_o2p_idxs1_data=const_cast<int*>(reper_o2p_idxs1->shaped<int,1>({pn1}).data());
        auto reper_p2o_idxs1_data=const_cast<int*>(reper_p2o_idxs1->shaped<int,1>({pn1}).data());
        ...
        computeRepermutationInfoImpl(o2p_idxs2_data,lens_data,begs_data,reper_lens_data,reper_begs_data,
                                     reper_cens_data,reper_o2p_idxs1_data,reper_p2o_idxs1_data,pn1,pn2);
