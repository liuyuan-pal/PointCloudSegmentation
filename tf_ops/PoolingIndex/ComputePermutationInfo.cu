...
__global__
void computeWholeIdxsWithClassKernel(
        int *voxel_idxs,         // [pn,3]
        int *classes,            // [pn]
        long *voxel_whole_idxs,  // [pn]
        int pn
)
{
    ...
    voxel_whole_idxs[pi]+=voxel_idxs[t+2];
    voxel_whole_idxs[pi]<<=16;
    voxel_whole_idxs[pi]+=classes[pi];
}
...
//    time_t bg=clock();
    ...
    // get whole idx
    thrust::device_vector<long> whole_idxs(pn);
    computeWholeIdxsWithClassKernel<<<block_dim,thread_dim>>>(voxel_idxs,classes,thrust::raw_pointer_cast(whole_idxs.data()),pn);
    fillIndex<<<block_dim,thread_dim>>>(origin2permutation_idxs,pn);
