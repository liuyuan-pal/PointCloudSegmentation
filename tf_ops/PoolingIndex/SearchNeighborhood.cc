...
REGISTER_OP("SearchNeighborhoodBruteForceRange")
        .Input("xyzs: float32")                 // [pn]
...
class SearchNeighborhoodBruteForceRangeGPUOp: public OpKernel
{
    float squared_min_nn_size;
    float squared_max_nn_size;
public:
    explicit SearchNeighborhoodBruteForceRangeGPUOp(OpKernelConstruction* context) : OpKernel(context)
    {
...
REGISTER_KERNEL_BUILDER(Name("SearchNeighborhoodBruteForceRange").Device(DEVICE_GPU), SearchNeighborhoodBruteForceRangeGPUOp);
