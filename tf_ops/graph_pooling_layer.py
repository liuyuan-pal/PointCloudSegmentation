...
def search_neighborhood(xyzs,radius):
    idxs,lens,begs=pooling_ops.search_neighborhood_brute_force(xyzs,squared_nn_size=radius*radius)
    return idxs,lens,begs
...
def points_pooling(xyzs,feats,labels,voxel_size=0.2,block_size=3.0):
    ...
    diff_pts = pooling_ops.compute_diff_xyz(xyzs, center_pts, voxel_idxs_cens)
