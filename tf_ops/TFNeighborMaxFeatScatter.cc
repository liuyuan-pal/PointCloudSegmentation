...
REGISTER_OP("NeighborMaxFeatScatter")
        ...
        .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
            ...
            TF_RETURN_IF_ERROR(c->WithRank(c->input(4),1,&vlens_shape));
...
public:
    ...
    void Compute(OpKernelContext* context) override
    {
        ...
        const Tensor& vlens=context->input(4);      // [pn2]
        ...
        OP_REQUIRES(context,vlens.dim_size(0)==pn2,errors::InvalidArgument("vlens dim 0"));
        ...
        auto vlens_p=reinterpret_cast<unsigned int*>(const_cast<int*>(vlens.shaped<int,1>({pn2}).data()));
        auto sfeats_p=sfeats->shaped<float,2>({pn1,fd}).data();

        neighborMaxFeatScatterGPU(gfeats_p,max_idxs_p,vlens_bgs_p,vlens_p,pn1,pn2,fd,sfeats_p);
    }
};
