...
from model import graph_conv_net,classifier
from train_util import *
from io_util import get_block_train_test_split,read_fn
from provider import Provider,default_unpack_feats_labels,default_batch_fn
...
def tower_loss(xyz,feats,labels,cidxs,nidxs,nidxs_lens,nidxs_bgs,m,
    ...
    with tf.variable_scope(tf.get_variable_scope(),reuse=reuse):
        global_feats,point_feats=graph_conv_net(xyz,feats,cidxs,nidxs,nidxs_lens,nidxs_bgs,m,pmiu,reuse,1024)

        global_feats=tf.expand_dims(global_feats,axis=0)    # [1,1024]
        k=tf.shape(point_feats)[0]
        global_feats=tf.tile(tf.expand_dims(global_feats,axis=1),[1,k,1])   #[pn,1024]
        all_feats=tf.concat([global_feats, point_feats, feats],axis=1)

        all_feats=tf.expand_dims(all_feats,0)   # [1,pn,2048+12]
        feats=tf.expand_dims(feats,0)           # [1,pn,12]
        logits=classifier(all_feats, feats, is_training, FLAGS.num_classes, reuse, use_bn=False)  # [1,pn,num_classes]
