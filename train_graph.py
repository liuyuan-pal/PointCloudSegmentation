...
from model import graph_conv_net_v6,classifier_v3
from train_util import *
from io_util import get_block_train_test_split,read_fn,get_class_names,get_class_loss_weights
...
parser.add_argument('--weighted_loss', type=bool, default=False, help='')
...
def tower_loss(xyz,feats,labels,cidxs,nidxs,nidxs_lens,nidxs_bgs,m,
               is_training,reuse=False,pmiu=None,weights=None):

    with tf.variable_scope(tf.get_variable_scope(),reuse=reuse):
        global_feats,global_pfeats,local_pfeats= \
            graph_conv_net_v6(xyz, feats, cidxs, nidxs, nidxs_lens, nidxs_bgs, m,
                                 pmiu, reuse, 1024, num_monitor=FLAGS.num_monitor)
        ...
        logits=classifier_v3(all_feats, local_pfeats, is_training, FLAGS.num_classes, reuse, use_bn=False)  # [1,pn,num_classes]
...
def train_ops(xyz, rgbs, covars, labels, cidxs, nidxs, nidxs_lens, nidxs_bgs, m,
              is_training, epoch_batch_num, pmiu=None,weights=None):
            ...
            with tf.device('/gpu:{}'.format(i)):
                with tf.name_scope('tower_{}'.format(i)):
                    ...
                    loss,logits=tower_loss(xyz[i], feats, labels[i], cidxs[i],
                                           ...
                                           m,is_training, reuse, pmiu, weights[i])
...
def train():
    ...
    try:
        ...
        ops=train_ops(pls['xyzs'],pls['rgbs'],pls['covars'],pls['lbls'],
                      ...
                      pmiu.shape[1],pls['is_training'],batch_num_per_epoch,pls['pmiu'],pls['weights'])
